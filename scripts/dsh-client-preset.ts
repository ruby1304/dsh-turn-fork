/**
 * Client bundle preset for UI plugin packages.
 *
 * Vendored from the DeepSeek Harness repository
 * (packages/client/tsdown.client.ts, MIT, © DeepSeek), slimmed for
 * registry-distributed third-party plugins: no workspace build faces, no
 * repository source-map rebasing. The bundle contract is unchanged: the
 * artifact calls `window.__ModuleLoader__.load({ id, factory })` and resolves
 * externals through the injected require (the loader module table). CSS
 * Modules are compiled by lightningcss inside the bundle: importing
...
/**
 * Mirrors `PLATFORM_MODULES` from @deepseek-ai/dsh-client-web: the module
 * specifiers the shell seeds into the frozen module table.
 */
